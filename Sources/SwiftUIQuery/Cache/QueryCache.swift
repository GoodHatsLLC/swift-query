...
public actor QueryCache {
    ...
    ///
    /// Deduplicates by data hash only - timestamp changes don't trigger new observations.
        ...
        AsyncStream { [dbPool] continuation in
            let observation = ValueObservation
                ...
                .removeDuplicates(by: { old, new in
                    // Only compare data hash - ignore timestamp/metadata changes
                    // This prevents loops: fetch → cache.set (new updatedAt) → observe → fetch...
                    old?.queryHash == new?.queryHash && old?.isStale == new?.isStale
                })
